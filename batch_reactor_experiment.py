...
class BatchReactorExperiment(Experiment):
    ...
    def create_model(self):
        if self.const_temp and not self.doe_solve:
            ...
            self.model = reform_batch_reactor_model_doe(self.XA0, self.temp_control)
    ...
    def label_model(self):
        ...
        if self.doe_solve:
            ...
            m.measurement_error.update(
                (m.XB[t], 1e-02) for t in m.t
            )
            m.measurement_error.update(
                (m.XC[t], 1e-02) for t in m.t
            )
            m.measurement_error.update(
...
def reform_batch_reactor_model_doe(XA0, temp_control_points):
    """
    ...
    model.XB = pyo.Var(model.t, bounds=(0, 1), initialize=1 - XA0 - 4e-6)
    model.XC = pyo.Var(model.t, bounds=(0, 1), initialize=1e-6)
    model.XE = pyo.Var(model.t, bounds=(0, 1), initialize=1e-6)
    model.XP = pyo.Var(model.t, bounds=(0, 1), initialize=1e-6)
    model.XG = pyo.Var(model.t, bounds=(0, 1), initialize=1e-6)

    # add the temperature variables
    model.T_reparam = pyo.Var(model.t, bounds=[1/6.8, 1/5.8], initialize=1/6)

    # add the rate constants
    model.k_reparam = pyo.Var(reaction_number, model.t, bounds=(0, None), initialize=1)
    model.k = pyo.Var(reaction_number, model.t, bounds=(0, None), initialize=10)
    ...
    # reaction rates
    model.r = pyo.Var(reaction_number, model.t, bounds=(0, None), initialize=1)

    # define the rate of reactions
    def reaction_rate_rule(m, i, t):
        if i == 1:
            return m.r[i, t] == m.k[i, t] * m.XA[t] * m.XB[t]
        elif i == 2:
            return m.r[i, t] == m.k[i, t] * m.XB[t] * m.XC[t]
        else:
            return m.r[i, t] == m.k[i, t] * m.XC[t] * m.XP[t]

    model.reaction_rate = pyo.Constraint(
        reaction_number, model.t, rule=reaction_rate_rule
    )
    ...
    for t in model.t:
        if t < 0.5:
            ...
            temp_at_time_t = temp_control_points["0.5"] + 0.6 * (t - 0.5)

        else:
            temp_at_time_t = 6.2

        model.T_reparam[t].fix(1 / temp_at_time_t)
